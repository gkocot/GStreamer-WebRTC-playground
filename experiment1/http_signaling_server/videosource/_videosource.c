...
<html> \n \
 ...
 \n \
      function onIncomingSDP(sdp) { \n \
        ...
        //sdp.sdp = sdp.sdp.replace(\"42801f\", \"42e01f\"); \n \
        //sdp.sdp = sdp.sdp.replace(\"sprop-parameter-sets=Z0KAH4uVAKALdCAAAXcAACvyEIA=,aN44gA==;\", \"\"); \n \
        //console.log(\"My Incoming SDP: \" + JSON.stringify(sdp)); \n \
 ...
 \n \
  <body> \n \
    <div> \n \
      <video controls muted id=\"stream\" autoplay playsinline>Your browser does not support video</video> \n \
    </div> \n \
  </body> \n \
</html> \n \
";
...
create_receiver_entry (SoupWebsocketConnection * connection)
{
  ...
  receiver_entry->pipeline =
      ...
      //GK EXAMPLE VIDEO SOURCE WORKS
      //"videotestsrc ! clockoverlay color=0"
      
      //GK PLAYING FROM CAMERA WORKS, BUT HOW COULD WE GET RID OF THE decode PLUGIN? CAMERA PRODUCES H264 STREAM, CAN WE JUST PASS IT THROUGH? 
      //"rtspsrc location=rtsp://admin:visio1234@192.168.5.13:554/Streaming/Channels/101?profile=Profile_1"
      "rtspsrc location=rtsp://admin:visio1234@192.168.5.13 short-header=true"
      //" ! decodebin "
      " ! rtph264depay "      
      //" ! video/x-h264, format=avc,alignment=au "
      //" ! avdec_h264 "    
      //" ! videorate ! videoscale ! video/x-raw,width=640,height=360,framerate=15/1 ! videoconvert ! queue max-size-buffers=1 "      
      ...
      //GK DISABLE AUDIO, PIPELINE IS NOT PLAYING WHEN THIS IS ENABLED, WHY?!
      // "autoaudiosrc"
  ...
  //GK REMOVE AS MUCH AS YOU CAN EXPERIMENT
#if 0
  receiver_entry->pipeline =
      gst_parse_launch ("webrtcbin name=webrtcbin stun-server=stun://"
      STUN_SERVER " "
      //GK PLAYING FROM CAMERA WORKS, BUT HOW COULD WE GET RID OF THE decode PLUGIN? CAMERA PRODUCES H264 STREAM, CAN WE JUST PASS IT THROUGH? 
      "rtspsrc location=rtsp://admin:visio1234@192.168.5.104:554/Streaming/Channels/101?profile=Profile_1 "
      // " ! decodebin "
      // " ! rtph264depay "
      //" ! avdec_h264 "
  ...
  //GK DEBUG
  GST_DEBUG_BIN_TO_DOT_FILE(receiver_entry->pipeline, GST_DEBUG_GRAPH_SHOW_ALL, "pipeline_ready");
...
// GK g_object_unref (G_OBJECT (soup_server));
// GK g_hash_table_destroy (receiver_entry_table);
